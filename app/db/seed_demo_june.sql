-- ============================================================
-- ⚠️【旧スキーマ用・実行済み・参考保存】 ⚠️
--   このスクリプトは旧 daily_sales（担当別1行）モデル前提で書かれており、
--   伝票モデル（sales_slips ＋ sales_slip_details）へ移行した現行スキーマでは
--   そのままでは実行できない（daily_sales / related_sale_id は存在しない）。
--   7月本番稼働前の6月デモ投入用に一度だけ使用した一発物。履歴として残置。
--   現行スキーマでデモを投入したい場合は seed_dashboard_demo.sql を使うこと。
-- ============================================================
